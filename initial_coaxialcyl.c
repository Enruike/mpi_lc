...
bool initial_coaxialcyl(){
	...
	//define the droplet //Agregué un if que determina si la condición de doble U está activa. 
	if(DoubleU){

        printf("Please, deactivate DoubleU mode!\n");
        exit(1);
...
//	printf("\ndV is %lf\ndA of droplet is %lf\ndA of nanoparticle is %lf\n", dV, dAdrop, dApart); 
	...
	dV = (M_PI * Rx * Rx * Lz - M_PI * iRx * iRx * Lz - 4.0 / 3.0 * M_PI * Rp * Rp * Rp * Np) / bulk; 
	//dAdrop = (2 * M_PI * R * Lz) / (surf -  nsurf);

	dAdrop = ( 2. * M_PI * Rx * Lz + 2. * M_PI * iRx * Lz) / (surf -  nsurf);
