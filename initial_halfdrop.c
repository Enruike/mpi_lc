...
bool initial_halfdrop(){
	...
	l = 0;
	//define the droplet. In this case is a half drop. 
	for(k = 0; k < Nz; k++){
		for (j = 0; j < Ny; j++){
			for (i = 0; i < Nx; i++){
		...
		for (j = 0; j < Ny; j++){
			for (i = 0; i < Nx; i++){
				if(drop[l]){
					...
					if(!drop[xm] || !drop[xp] || !drop[ym] || !drop[yp] || !drop[zm] || !drop[zp] || j == 2){
						boundary[l] = true;
						surf ++;
					}
...
//	printf("\ndV is %lf\ndA of droplet is %lf\ndA of nanoparticle is %lf\n", dV, dAdrop, dApart); 
	...
	dV = (2.0 / 3.0 * M_PI * R * R * R - 4.0 / 3 * M_PI * Rp * Rp * Rp * Np) / bulk; 
	dAdrop = (2 * M_PI * R * R + M_PI * R * R) / (surf -  nsurf);
