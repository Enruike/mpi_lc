		...
		for (j = 0; j < Ny; j++){
			for (i = 0; i < Nx; i++){
				if(drop[l]){
					...
					if(!drop[xm] || !drop[xp] || !drop[ym] || !drop[yp] || j == 2 || i == 2){
						boundary[l] = true;
						surf ++;
					}					
				}
...
//	printf("\ndV is %lf\ndA of droplet is %lf\ndA of nanoparticle is %lf\n", dV, dAdrop, dApart); 
	...
	dV = (0.5 * 0.5 * M_PI * Rx * Rx * Lz - 4.0 / 3.0 * M_PI * Rp * Rp * Rp * Np) / bulk; 
	//dAdrop = (2 * M_PI * R * Lz) / (surf -  nsurf);
	dAdrop =( ( 0.5 * M_PI * Rx * Lz) + 2.0 * Rx * Lz) / (surf -  nsurf);
	...
	pnorm = sqrt(dir2[0] * dir2[0] + dir2[1] * dir2[1] + dir2[2] * dir2[2]);
	dir2[0] = dir2[0] / pnorm;
	dir2[1] = dir2[1] / pnorm;
	dir2[2] = dir2[2] / pnorm;

	pnorm = sqrt(dir1[0] * dir1[0] + dir1[1] * dir1[1] + dir1[2] * dir1[2]);
	dir1[0] = dir1[0] / pnorm;
	dir1[1] = dir1[1] / pnorm;
	dir1[2] = dir1[2] / pnorm;
	
	//printf("dir1[0] = %lf, dir1[0] = %lf, dir1[0] = %lf\n", dir1[0], dir1[1], dir1[2]);
	//printf("dir2[0] = %lf, dir2[0] = %lf, dir2[0] = %lf\n", dir2[0], dir2[1], dir2[2]);
	...
	for(int nd = 0; nd < droplet; nd ++){
		...
		else if((share[nd] < 8 && share[nd] >= 2) || share[nd] == 12){	
			for(n = 0; n < 6; n++){
