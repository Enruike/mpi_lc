...
bool initial_sandwich(){
	...
	if(DoubleU){
		//Omitiremos las superficies. k = 1 y k = Nx - 1;
		for(int k = 0; k < Nz; k++){
			for(int j = 0; j < Ny; j++){
				for(int i = 0; i < Nx; i++){
					if(k >= iRz - 1 && k <= Nz - iRy - 1){
						init_bulktype[i + j * Nx + k * Nx * Ny] = 1;
						bulkin++;
					}
					else{
						init_bulktype[i + j * Nx + k * Nx * Ny] = 2;
						bulkout++;
					}
	...
	if(DoubleU){
		dVi = ((Lz - iRy - iRz) * Lx * Ly) / bulkin; 
		dVo = ((iRz * Lx * Ly) + (iRy * Lx * Ly)) / (bulkout - surf);
	}
					...
					if(boundary[l]){
						...
						if(DoubleU){
							if(infinite == 1){
								if(k == 0){
									for(n = 0; n < 6; n ++){
										Qold[nd * 6 + n] = dir2ten(dir1, n, S2);
									}	
								}	
								else{
									for(n = 0; n < 6; n ++){
										Qold[nd * 6 + n] = dir2ten(dir2, n, S2);
									}	
								}
							}
								...
								if(k == 0){
									for(n = 0; n < 6; n ++){
										Qo[nb * 6 + n] = dir2ten(dir1, n, S2);
								//		Qold[nd * 6 + n] = dir2ten(dir1, n, S);
									}		
								}
								else{
									for(n = 0; n < 6; n ++){
										Qo[nb * 6 + n] = dir2ten(dir2, n, S2);
...
*/
	...
	printf("Initialization of sandwich successfully.\n");
	return true;
}
